...
it('should show error when cannot find rss', (done) => {
  fetchTimeout.mockResolvedValue({ json: jest.fn().mockResolvedValue({ type: 'website', name: 'Example', logo: 'https://example.com/logo.png', rss: [] })});
  const wrapper = mount(DaNewSource, { store, localVue, apolloProvider: new VueApollo({defaultClient: apolloClient }) });
  const input = wrapper.find('input');
  input.element.value = 'https://example.com';
  input.trigger('input');
...
it('should list scraped rss feeds', (done) => {
  fetchTimeout.mockResolvedValue({ json: jest.fn().mockResolvedValue({ type: 'website', name: 'Example', logo: 'https://example.com/logo.png', rss: [{title: 'A', url: 'https://a.com/feed'}, {title: 'B', url: 'https://b.com/feed'}] })});
  const wrapper = mount(DaNewSource, { store, localVue, apolloProvider: new VueApollo({defaultClient: apolloClient }) });
  const input = wrapper.find('input');
  input.element.value = 'https://example.com';
  input.trigger('input');
...
it('should proceed directly to source info when one rss is available', (done) => {
  fetchTimeout.mockResolvedValue({ json: jest.fn().mockResolvedValue({ type: 'website', name: 'Example', logo: 'https://example.com/logo.png', rss: [{title: 'A', url: 'https://a.com/feed'}] })});
  const wrapper = mount(DaNewSource, { store, localVue, apolloProvider: new VueApollo({defaultClient: apolloClient }) });
  const input = wrapper.find('input');
  input.element.value = 'https://example.com';
  input.trigger('input');
...
it('should show source exists message', async () => {
  fetchTimeout.mockResolvedValue({ json: jest.fn().mockResolvedValue({ type: 'website', name: 'Example', logo: 'https://example.com/logo.png', rss: [{title: 'A', url: 'https://a.com/feed'}] })});
  const wrapper = mount(DaNewSource, { store, localVue, apolloProvider: new VueApollo({defaultClient: apolloClient }) });
  wrapper.setData({ existsSource: { id: 'source', name: 'Source', image: 'https://source.com/logo.png', public: true }});
